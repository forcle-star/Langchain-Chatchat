...
from configs import LLM_MODELS, TEMPERATURE, HISTORY_LEN, Agent_MODEL, MODEL_ROOT_PATH
...
#from server.agent_1.custom_agent.ChatGLM3Agent import initialize_glm3_agent
from server.agent.custom_agent.ChatGLM3Agent import ChatGLM3,initialize_agent_glm3
...
async def agent_chat(query: str = Body(..., description="用户输入", examples=["恼羞成怒"]),
    ...
    async def agent_chat_iterator(
        ...
        if "chatglm3" in model_container.MODEL.model_name or "zhipu-api" in model_container.MODEL.model_name:
            llm = ChatGLM3()
            llm.load_model(model_name_or_path=os.path.join(MODEL_ROOT_PATH, Agent_MODEL))
            agent_executor = initialize_agent_glm3(llm)
            print("************agent_executor**************")
            # print("***********************chatglm3*********************************/n",tools)
        ...
        while True:
            try:
                ...
                #     agent_executor.acall(query, callbacks=[callback], include_run_info=True))
                print("************************task********************",task)
                # print("************************callback.aiter()********************",callback.aiter())
                break
                # result = agent_executor.acall(query, include_run_info=True)
                
                # print("***********result************",type(result),result[2])
                # break
            except:
                pass
        # yield json.dumps(result[2])
        # await result
        print("************************stream********************",stream)
        if stream:
